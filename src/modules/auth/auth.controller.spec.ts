...
describe('AppController', () => {
  ...
  it('Should allow signup email and password', async () => {
    jest
      ...
      .mockImplementation(async (slug: string, data: UserSignupDto) => {
  ...
  it('should allow user login with password and email', async () => {
    jest
      ...
      .mockImplementation(async (user: Users, slug) => {
        return { token: 'some dummy token' };
      });

    const res = await authController.LocalLogin(
      { user: users[1] } as AuthRequest,
      'platform',
    );
  ...
  it('should allow google login', async () => {
    const validateUserMockSpy = jest
      .spyOn(authService, 'validateUser')
      .mockImplementation(async (data, type: 'google' | 'local') => {
    ...
    const loginUserMockSpy = jest
      .spyOn(authService, 'login')
      .mockImplementation(async (user: Users, slug: string) => {
        return { token: 'some dummy token' };
      });

    const res = await genAuthController.GoogleLogin(
      { user: { email: 'engr.omoalfa@gmail.com' } } as unknown as Request,
      '{"slug": "platform"}',
    );
