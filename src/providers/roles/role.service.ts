...
class RoleServies {
  ...
  public createRoles = async (
    ...
    const scopes = data.scopes.reduce(
      (prev, cur) =>
        prev + `:::${cur.replace('{ORG_SLUG}', organization.slug)}`,
      '',
    );
  ...
  public getScopes = async (organization: Organization) => {
    const scopes = Object.values(EScopes).map((s) =>
      s.replace('{ORG_SLUG}', organization.slug),
    );

    return scopes;
  };

  public addScope = async (
    org: Organization,
    id: number,
    ...
    const scopes = raw.reduce(
      (prev, cur) => prev + `:::${cur.replace('{ORG_SLUG}', org.slug)}`,
      role.scopes,
    );
  ...
  public removeScope = async (
    ...
    const newScopesArray = currentScopesArray.filter(
      (scope) => !toRemoveScopes.includes(scope),
    );

    const scopes = newScopesArray.reduce(
      (prev, cur) => prev + `:::${cur.replace('{ORG_SLUG}', org.slug)}`,
      '',
    );
